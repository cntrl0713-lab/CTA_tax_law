 ...
 *
 * 아직 검증되지 않은 3가지 가설을 테스트한다:
 *   실험 1 (같은 물음 내 다른 루브릭 오염) — 물음1은 루브릭 3개를 한 호출에서 함께 채점한다.
 *     문제의 루브릭("재산가치 증가사유 발생 요건")만 단독으로 담은 미니 물음으로 떼어내 채점해도
 *     재현되는지 확인 — 재현되면 "물음 단위 격리"만으로는 못 잡는, 같은 물음 안 다른(완전한)
 *     루브릭들이 만드는 "이 답안은 충실하다"는 인상 전이 가설을 확인/기각한다.
 *   실험 2 (thinkingBudget 부족) — 현재 gradeSingleSubquestion은 thinkingBudget:1024로 고정.
 *     동일 프롬프트·스키마에서 thinkingBudget만 8192로 올려도 결과가 달라지는지 확인 —
 *     프롬프트 문구가 아니라 추론 여유 부족이 원인일 가능성을 테스트한다(README에 없던 축).
 *   실험 3 (국소적 요소분해 유도) — README 실험④(전역 스키마에 elementCheck 필드 강제)는
 *     이 루브릭은 고쳤지만 무관한 OR형 루브릭에 회귀를 유발해 원복됐다. 전역 스키마 변경 없이,
 *     이 물음 하나의 호출에만 "핵심 요건을 요소별로 나열한 뒤 각각 답안에 있는지 확인하라"는
 *     자유 텍스트 사고 유도 문장을 추가하면(응답 스키마는 그대로) 같은 효과가 나는지, 즉
 *     "스키마 강제"가 문제였는지 "요소분해 지시 자체"가 무력한지 분리한다.
 *
 * 실행 (CTA_tax_law 디렉터리):
