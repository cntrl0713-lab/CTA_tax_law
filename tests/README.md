...
`--ambiguous` 실행 결과: 문제1 19/25=76%, 문제9 17/20=85%로 전부 통과한다. 문제46은 총점 22/25=88%로 아깝게 통과하지만, 물음3 "감면대상 소득비율 산출" 루브릭 하나가 개별 체크에서 FAIL — 답안에 "40/60=66.6%(2/3)"라는 산출식과 결과값이 명시되어 있는데도 모델이 unmet(0점) 처리한 사례. 코드 버그가 아니라 모델이 완곡한 문장 속 수치를 놓친 것으로 보이며, `tests/test.md`에 기록해 두었다.
