...
문제 1(재산가치 증가이익의 증여, 상속세및증여세법)과 문제 51(세무조사권 남용, 국세기본법) 두 개의 서로 다른 과목·구조 픽스처를 내장하고 있다. 문제를 하나만 쓰면 그 문제의 특정 문구에 과적합된 수정을 놓칠 수 있어서 처음부터 두 개를 유지했다.

```bash
cd CTA_tax_law
npx -y tsx --env-file=.env.local scripts/verify-grading.ts                  # 문제1 충실한 답안 (기본)
npx -y tsx --env-file=.env.local scripts/verify-grading.ts --incomplete      # 문제1 관대화 방지
npx -y tsx --env-file=.env.local scripts/verify-grading.ts --half            # 문제1 절반 답안 비례성
npx -y tsx --env-file=.env.local scripts/verify-grading.ts --problem51       # 문제51 강한 답안
npx -y tsx --env-file=.env.local scripts/verify-grading.ts --problem51 --incomplete
```
