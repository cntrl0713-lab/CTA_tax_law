...
export default async function NotesPage() {
    ...
    const { data: notes } = await admin
        ...
        .select(`
            ...
            result_json,
            cta_problem!inner ( id, title, total_score, subject_id,
    ...
    const typedNotes = (notes || []) as unknown as (GradingAttempt & {
        cta_problem: { id: number; title: string; total_score: number; cta_subject: { id: number; name: string } }
    })[]
                    ...
                    {typedNotes.map((note) => {
                        ...
                        return (
                            <Link
                                ...
                                className="note-card"
                            >
                                <div className="note-card-subject">{subject?.name ?? '세법'}</div>
